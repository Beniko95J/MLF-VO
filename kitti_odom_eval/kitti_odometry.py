...
class KittiEvalOdom():
    ...
    def plot_trajectory(self, poses_gt, poses_result, seq):
        ...
        fig.savefig(fig_pdf, bbox_inches='tight', pad_inches=0)
        plt.close(fig)
    ...
    def eval(self, gt_dir, result_dir, 
        ...
        for i in self.eval_seqs:
            ...
            poses_result = KittiEvalOdom.load_poses_from_txt(result_dir+"/"+file_name)
            poses_gt = KittiEvalOdom.load_poses_from_txt(self.gt_dir + "/" + file_name)
            ...
            if alignment == "scale":
                poses_result = KittiEvalOdom.scale_optimization(poses_gt, poses_result)
            ...
            ave_t_errs.append(ave_t_err*100)
            ave_r_errs.append(ave_r_err/np.pi*180*100)
            terr_sum += ave_t_err*100
            rerr_sum += ave_r_err/np.pi*180*100
        ...
        if not silent:
            print('')
            print('Average Translational error (%): ', terr_sum / len(self.eval_seqs))
            print('Average Rotational error (deg/100m): ', rerr_sum / len(self.eval_seqs))
