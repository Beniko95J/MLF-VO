...
class Model_multimodal(BaseModel):
    ...
    def load_model(self, path: str) -> None:        
        ...
        self.model['pose'] = posenet18(num_parallel=2, bn_threshold=2e-2, r_type='euler').eval()
        # self.model['pose'] = posenet18(r_type='euler').eval()
    ...
    def infer_pose(self, img1: torch.Tensor, img2: torch.Tensor, disp1: torch.Tensor, disp2: torch.Tensor) -> np.ndarray:
        ...
        T = transformation_from_parameters(axisangle[:, 0], translation[:, 0], invert=True, type='euler')
        T = T.cpu().numpy()[0]
