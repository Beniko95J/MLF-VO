...
from evaluate_odometry import VisualOdometry
...
class Trainer:
    def __init__(self, options):
        ...
        # self.models["pose"] = networks.posenet18(r_type='euler')
        ...
        for name, param in self.models["pose"].named_parameters():
            # if param.requires_grad and name.endswith('weight') and 'bn2' in name and ('layer3' in name or 'layer4' in name):
        ...
        self.seqs = [7, 9, 10]
        self.cfg = edict()
        self.cfg.model_type = 'multimodal'
        self.cfg.data_dir = '/home/jiang/Documents/datasets/kitti_odom/dataset'
        self.cfg.gt_dir = '/home/jiang/Documents/datasets/kitti_odom/dataset/poses'        
        self.cfg.do_vis = False
        ...
        for self.epoch in range(self.opt.num_epochs):
            ...
            for seq in self.seqs:
                ...
                self.cfg.output_dir = os.path.join(
                    self.log_path, "models", "weights_{}".format(self.epoch))
                vo = VisualOdometry(self.cfg)
                vo.main()

            ave_t_errs, ave_r_errs = self.eval_tool.eval(
                self.cfg.gt_dir,
                self.cfg.output_dir,
                alignment='7dof', 
                seqs=self.seqs,
            ...
            errs = {}
            errs['Seq07/t_rel'] = ave_t_errs[0]
            errs['Seq07/r_rel'] = ave_r_errs[0]
            errs['Seq09/t_rel'] = ave_t_errs[1]
            errs['Seq09/r_rel'] = ave_r_errs[1]
            errs['Seq10/t_rel'] = ave_t_errs[2]
            errs['Seq10/r_rel'] = ave_r_errs[2]
            errs['test/t_rel'] = (ave_t_errs[1] + ave_t_errs[2]) / 2
            errs['test/r_rel'] = (ave_r_errs[1] + ave_r_errs[2]) / 2
            self.log_errors(errs)
    ...
    def compute_bn_regularization(self):
        return sum([L1_penalty(m).cuda() - 0.01 * polorize(m).cuda() for m in self.slim_params])
