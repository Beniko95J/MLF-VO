...
class PoseNet(nn.Module):
    ...
    def __init__(self, block, layers, num_parallel, bn_threshold=2e-2, r_type='euler'):
        ...
        if r_type == 'axisangle':
            self.fc3 = ModuleParallel(nn.Linear(512, 6)) # FIXME
        elif r_type == 'euler':
    ...
    def forward(self, x):
        ...
        # alpha_soft = self.alpha(_x)
        # beta_soft = self.beta(_x)
        # alpha_soft = self.sigmoid(alpha_soft)
        # beta_soft = self.sigmoid(beta_soft)
        # alpha_soft = [alpha_soft[0] + 1e-9, alpha_soft[1] + 1e-9]
        # beta_soft = [beta_soft[0] + 1e-9, beta_soft[1] + 1e-9]
        # alpha_soft = [alpha_soft[0] / (alpha_soft[0] + alpha_soft[1]), alpha_soft[1] / (alpha_soft[0] + alpha_soft[1])]
        # beta_soft = [beta_soft[0] / (beta_soft[0] + beta_soft[1]), beta_soft[1] / (beta_soft[0] + beta_soft[1])]
